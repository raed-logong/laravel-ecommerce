...
@endif
      @php($user=auth()->user())
    <img class="mr-3" src="{{asset('storage/'.$comment->commenter->avatar)}}" width="50" height="50" alt="{{ $comment->commenter->name ?? $comment->guest_name }} Avatar">
    <div class="media-body">
        ...
        @can('edit-comment', $comment)
            <div class="modal" id="comment-modal-{{ $comment->id }}" tabindex="-1" role="dialog">
        ...
        @can('reply-to-comment', $comment)
            <div class="modal" id="reply-modal-{{ $comment->id }}" tabindex="-1" role="dialog">
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        <form method="POST" action="{{ url('comments/' . $comment->id) }}">
                            @csrf
